...
if %1 == 1 (echo Making Fourses Windows DEMO VERSION distribution ...) else (echo Making Fourses Windows FULL VERSION distribution ...)
...
REM - msbuild Fourses-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Fourses.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Fourses.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Fourses.aaxplugin\Contents\Win32\Fourses.aaxplugin --out .\build-win\aax\bin\Fourses.aaxplugin\Contents\Win32\Fourses.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Fourses.aaxplugin\Contents\x64\Fourses.aaxplugin --out .\build-win\aax\bin\Fourses.aaxplugin\Contents\x64\Fourses.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Fourses.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Fourses.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Fourses Installer" ".\installer\Fourses Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Fourses Installer.exe" ".\installer\Fourses Demo Installer.exe"
REM -del ".\installer\Fourses Installer.exe"
REM -)
