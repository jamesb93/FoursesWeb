...
def  main():
  ...
  installer = "\installer\Fourses Installer.exe"
   
  if demo:
    installer = "\installer\Fourses Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Fourses manual.pdf" 
  ]
  ...
  ZIPNAME = "Fourses-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Fourses-v" + FULLVERSIONSTR + "-win-demo.zip"
